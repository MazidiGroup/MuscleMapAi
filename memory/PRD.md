...
- PHASE 6 (exercise animations) — RepDB pack purchased by user (perpetual commercial license, safe to bundle).
  ...
  - Assets: pack parts 1-5 of 7 imported (478-exercise SQLite + 345 webp anims, 512px, ~1.4MB ea). Mapping script /app/backend/scripts/map_repdb.py (our 32 ids -> RepDB ids) copied 25 animations (37MB) into static/exercise_media. MISSING (awaiting parts 6-7): tricep-pushdown, squat, walking-lunge (lunge), standing-calf-raise, plank + the 1024px transparent poster images. NOT IN REPDB (icon fallback stays): cuban-rotation, tibialis-raise. Full pack kept at /tmp/repdb/pack (tmp — re-download from chat artifacts if wiped).
