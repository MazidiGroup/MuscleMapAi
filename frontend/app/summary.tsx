      ...
      <View style={styles.panel}>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: insets.bottom + 80 }}>
          ...
          <View style={styles.statGrid}>
            <Stat label="Duration" value={fmt(workout.durationSec)} icon="time" />
            <Stat label="Exercises" value={`${workout.exercises.length}`} icon="list" />
            <Stat label="Sets" value={`${stats.completed}`} icon="layers" />
            <Stat label="Reps" value={`${stats.reps}`} icon="repeat" />
            <Stat label="Volume" value={`${stats.volume} kg`} icon="barbell" />
            <Stat label="Muscles" value={`${act.list.length}`} icon="body" />
          </View>
...
const styles = StyleSheet.create({
  ...
  prCard: { backgroundColor: "rgba(255,176,32,0.12)", borderWidth: 1, borderColor: T.secondary + "55", borderRadius: 14, padding: 14, marginTop: 14 },
  ...
  track: { height: 8, borderRadius: 4, backgroundColor: T.surfaceHi, overflow: "hidden" },
  ...
  doneBtn: { position: "absolute", left: 18, right: 18, backgroundColor: T.accent, borderRadius: 14, paddingVertical: 15, alignItems: "center" },
